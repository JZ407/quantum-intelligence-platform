...
**修复**：`generate_weekly_report.py` 的 `classify_news()` 增加对 dict 格式的兼容：
- 检测 `isinstance(tags, dict)` → 提取 `tags['weekly']` 获取分类标签
- 保留对旧格式 flat list 的向后兼容

**验证**：重生成第 11 期周报（2026-05-23 ~ 2026-05-29），分类结果：
...
**修复**：`daily_report_app.py` 中 `HISTORICAL_DB_PATH` 从 `historical_v2.db` 切换为 `historical_final.db`。

**端口僵尸清理**：8501 端口累计 37 个 Streamlit 僵尸进程（每次重启未正确终止旧进程），8502 2 个、8503 1 个。共清理 41 个进程，释放约 1GB 内存。重新启动后仅一个干净进程运行在 8501。
...
## 20.43 反爬保护：随机延迟 + UA 轮换 (2026-06-04)

> 📦 `liangke_daily: 97db2c1`

**问题**：三子页策略将每日请求从 ~16 增至 ~171，无延迟连续请求极易触发反爬。

**措施**：

| 层级 | 机制 | 说明 |
|------|------|------|
| 请求间隔 | `_polite_delay(1-3s)` | 每页列表页之间、每篇详情页之间随机延迟 |
| UA 轮换 | `_rotate_ua()` | 4 个桌面浏览器 UA（Chrome 130/131、Firefox 133、Safari 18.2）随机切换 |
| 请求节奏 | 先收集再详情 | 列表页全部跑完 → 再逐个详情，不会同一 URL 模式连续轰炸 |
...
- DB 总量：2,992 → **3,246**（+254）
- 含 Blog 10 篇（avg 5,991 字）

**内容质量修复**（`6576370`）：
- 停止词 `D-Wave Quantum Inc.` 出现在正文中误截断 170 篇 → 改用智能匹配（仅短行标题触发）
- 修复后 press avg 4,263 字（原 1,454），<1000 字比从 73% 降至 8.6%

**机构新闻累计**：16 家机构、30 个源、3,246 篇
